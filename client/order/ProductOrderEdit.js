...
export default function ProductOrderEdit(props) {
    ...
    useEffect( () => {
        ...
        getStatusValues(signal)
            .then( data => {
                if (data.error) setValues({...values, error: 'Could not get status'})
                else setValues({...values, setValues: data, error: ''})
            })
    ...
    const handleStatusChange = (event, productIndex) => {
        ...
        if (event.target.value == 'Cancelled') {
            ...
            .then( data => {
                if (data.error) setValues({...values, error: 'Status not updated, try again'})
                else {
                    props.updateOrders( porps.orderIndex, order)
            ...
            .then( data => {
                if (data.error) setValues({...values, error: 'Sttaus not updated, try again'})
                else {
                    props.updateOrders(props.orderIndex, order)
                    setValues({...values, error: ''})
                }
