...
const styles = theme => ({
    ...
    class Order extends Component {
        ...
        componentDidMount = () => {
            read({ orderId: this.match.params.orderId })
            .then( data => {
                if (data.error) this.setState({ error: data.error })
                else this.setState({ order: data })
            })
        }

        getTotal() {
            return this.state.order.products.reduce( (acc, cur) => {
                const quantity = cur.status == 'Cancelled' ? 0 : cur.quantity
                return acc + ( quantity * cur.product.price )
            }, 0)
        }
        ...
        return (
            <Card className={classes.card}>
                ...
                <Typography type="subheading" component="h2" className={classes.subheading}>
                    Order Code: <strong>{this.state.order._id}</strong> <br/>
                    Placed on {(new Date(this.state.order.created)).toDateString()}
                </Typography><br/>
                <Grid container spacing={1}>
                    ...
                    <Grid item xs={5} sm={5}>
                        <Card className={classes.innerCard}>
                            ...
                            <Typography type="subheading" component="h3" className={classes.info} color="primary">
                                <strong>{this.state.order.customer_name}</strong>
                            </Typography><br/>
                            <Typography type="subheading" component="h3" className={classes.info} color="primary">
                            ...
                            <Typography type="subheading" component="h3" className={classes.itemShop} color="primary">
                                {this.state.order.delivery_address.city}, {this.state.order.delivery_address.state} {this.state.order.delivery_address.zipcode}
                            </Typography>
                            <Typography type="subheading" component="h3" className={classes.itemShop} color="primary">
                                {this.state.order.delivery_address.country}
                            </Typography><br/>
                            <Typography type="subheading" component="h3" className={classes.thanks} color="primary">
                                Thank you for shopping with us! <br/>You can track the status of your purchased items on this page.
                            </Typography>
                        </Card>
                    </Grid>
                </Grid>
